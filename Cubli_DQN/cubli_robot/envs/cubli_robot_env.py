...
class Cubli_Env(gym.Env):
    ...
    def __init__(self, render=False):
        self._observation = []
        self.action_list = [-10.0, -5.0, -2.0, -1.0, -0.5, -0.1, 0, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0]
        self.action_space = spaces.Discrete(13)    # 13 discrete actions
        self.observation_space = spaces.Box(np.array([-math.pi, -math.pi, -200]),
                                            np.array([math.pi, math.pi, 200]))
    ...
    def _pybullet_control(self, action):
        self.deltav = np.array(self.action_list)[np.array(action)]# problem
